...
GPT-5.5 at medium/high effort for multi-file research and high-risk review. These assignments are
testable hypotheses, not efficiency claims.

## Availability and fallback

Model availability is specific to the installed Codex version, account, workspace, and runtime.
...
Run GPT-5.5, Terra, Luna, or current equivalents on comparable issue categories before changing the
default. Record one row per completed issue:
...
Codex and Claude do not expose a reliable, comparable per-task subscription-unit counter in the
locally inspected configuration surfaces. Record visible limits or usage only as approximate data,
