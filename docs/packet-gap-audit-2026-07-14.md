...
- `PetItemList` and `PetStatusUpdate` remain deferred: the current Go pet actor still lacks the full pet item-list and status-delta snapshot surface needed to emit truthful packets for either. `PetInfo` is now wired: a pet or servitor spawning beside its owner (`summon.SpawnBesideOwner`) sends its owner (and only its owner — a non-owner observer would get `SummonInfo`, not ported yet) the full owner-window snapshot (position, combat stats, HP/MP/fed, level/exp, weight, mountable) built in `network/visibility.go`'s `petInfoSnapshot`. A pet's own carried-item inventory (`PetConfig.Inventory`) is never populated in production today, so `TotalWeight`/`WeightLimit` send `0`/`0` rather than the CON-derived limit (#1237). `PetStatusShow` and `PetDelete` are wired where the current runtime has exact backing behavior.
