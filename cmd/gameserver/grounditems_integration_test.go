...
// shutdown hook startGroundItemPersistence installs — the exact sequence
...
func TestGroundItemsBootWiringRoundTrip(t *testing.T) {
	...
	state := world.New()
	items, store, err := provideGroundItems(state, opts, db, data, log)
	if err != nil {
	...
	// startGroundItemPersistence installs (store.Save(ctx, items.Snapshots(nil))).
	...
	nextState := world.New()
	nextItems, _, err := provideGroundItems(nextState, opts, db, data, log)
	if err != nil {
