...
func TestLoadItemTemplates(t *testing.T) {
	...
	t.Run("etc item shot override beats an explicit etcitem_type", func(t *testing.T) {
		...
		if len(tpl.UseConditions) != 0 {
			t.Fatalf("item 1400 UseConditions = %+v, want none: use_condition is not a parsed item condition", tpl.UseConditions)
		}
	})
...
func TestLoadItemTemplatesAgainstDatapack(t *testing.T) {
	...
	t.Run("Lesser Healing Potion (1060): reuse group and delay", func(t *testing.T) {
		...
		if len(tpl.UseConditions) != 0 {
			t.Fatalf("item 1060 UseConditions = %+v, want none: use_condition is not a parsed item condition", tpl.UseConditions)
		}
	})
}
