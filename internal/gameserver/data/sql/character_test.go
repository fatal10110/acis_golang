...
func TestCharacterStoreSavePersistsFullStats(t *testing.T) {
	...
	if !strings.Contains(rec.query, "SET level = ?, maxHp = ?, curHp = ?, maxCp = ?, curCp = ?, maxMp = ?, curMp = ?, exp = ?, sp = ?, karma = ?, pvpkills = ?, pkkills = ?") {
		t.Fatalf("Save() query = %q, missing full-stat columns", rec.query)
	}
	want := []any{int64(5), 200.0, 150.0, 90.0, 45.0, 70.0, 60.0, int64(12345), int64(67), int64(240), int64(3), int64(1), int64(0x10000001)}
