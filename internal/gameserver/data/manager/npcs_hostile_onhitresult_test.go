...
func TestNewLiveHostileWiresOnHitResultFromEffects(t *testing.T) {
	...
	hostile, _, err := newLiveHostile(inst, 100, blockedHomeGeo{}, positions, zerolog.Nop(), fakeHostileCastDefs{}, effects, nil, 20, nil)
	if err != nil {
...
func TestNewLiveHostileLeavesOnHitResultUnsetWithoutCastDefs(t *testing.T) {
	...
	hostile, _, err := newLiveHostile(inst, 100, blockedHomeGeo{}, positions, zerolog.Nop(), nil, actorcast.EffectHandlers{}, nil, 20, nil)
	if err != nil {
		t.Fatalf("newLiveHostile() error: %v", err)
	}
