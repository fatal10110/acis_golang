...
func TestLiveHostileMoveHomeTeleportsThroughLocatedRef(t *testing.T) {
	...
	hostile, walkerRef, err := newLiveHostile(inst, 100, blockedHomeGeo{}, positions, zerolog.Nop(), nil, actorcast.EffectHandlers{}, nil, 20, nil, nil, nil)
	if err != nil {
...
func TestHostileControlClosesAbortedCastWithCancelAnimation(t *testing.T) {
	...
	state := world.New()
	hostile, _, err := newLiveHostile(inst, 100, blockedHomeGeo{}, task.NewPositionUpdates(state), zerolog.Nop(), nil, actorcast.EffectHandlers{}, nil, 20, nil, nil, nil)
	if err != nil {
		t.Fatalf("newLiveHostile() error: %v", err)
	}
	rec := &event.Recorder{}
