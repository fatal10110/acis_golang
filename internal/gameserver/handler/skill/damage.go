...
func (chargeDamHandler) UseResult(cast Cast) Result {
	...
	for _, obj := range cast.Targets {
		...
		applyChargeDamEffects(cast, obj, in.Shield)
	...
	for _, obj := range cast.Targets {
		...
		if in.Landed {
			...
			if damage > 0 {
				...
				applyBlowEffects(cast, obj, in.Shield, countered)
			}
...
func applyBlowEffects(cast Cast, obj any, shield formulas.ShieldDefense, countered bool) {
	if len(cast.Skill.Effects) == 0 {
		return
	}
	effected, reflected := reflectEffectTarget(cast, obj)
...
func applyChargeDamEffects(cast Cast, obj any, shield formulas.ShieldDefense) {
	if len(cast.Skill.Effects) == 0 {
		return
	}
	effected, reflected := reflectEffectTarget(cast, obj)
