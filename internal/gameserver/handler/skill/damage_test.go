...
func TestPdamAndChargeDamCounterSkillDamageCaster(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			health.hp = 2000
			target := newDamageEffectFake()
			target.hp, target.physicalOK, target.physicalInput, target.counterSkillPhysical, target.reflects = 2000, true, input, 50, true
			...
			if health.hp != 2000-want || target.hp != 2000 || len(result.Counterattacks) != 1 || len(health.EffectList().All()) != 1 || len(target.EffectList().All()) != 0 {
				t.Fatalf("counter %s hp = %v/%v, reports/effects = %d/%d/%d; want %v/2000, 1/1/0", tc.name, health.hp, target.hp, len(result.Counterattacks), len(health.EffectList().All()), len(target.EffectList().All()), 2000-want)
			}
		})
	}
}
