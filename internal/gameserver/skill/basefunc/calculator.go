...
// Calculator dynamically computes the effect of a Stat: a slice of Funcs
// kept sorted by Order, lowest first, with same-order Funcs applied in
...
func (c *Calculator) AddFunc(fn Func) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Calculator) RemoveFunc(fn Func) {
	c.mu.Lock()
	defer c.mu.Unlock()
