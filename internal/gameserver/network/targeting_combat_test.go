...
// TestCastFinishClearsRecentFakeDeath covers PlayerCast.doCast's
// unconditional _actor.clearRecentFakeDeath() (PlayerCast.java:181-185),
// wired through the cast controller's SetOnFinish observer at
// live_player.go. A natural finish clears the grace; an aborted/stopped
// cast, matching Java never reaching that line on interrupt, does not.
func TestCastFinishClearsRecentFakeDeath(t *testing.T) {
	def := modelskill.Definition{
	...
	tests := []struct {
		...
		{name: "natural finish clears grace", end: func(c *actorcast.Controller) { c.Finish() }, wantCleared: true},
		{name: "abort leaves grace running", end: func(c *actorcast.Controller) { c.Stop() }, wantCleared: false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			attacker.MarkRecentFakeDeath()
			if !attacker.RecentFakeDeath() {
			...
			if got := attacker.RecentFakeDeath(); got != !tt.wantCleared {
				t.Fatalf("RecentFakeDeath() = %v after %s, want cleared=%v", got, tt.name, tt.wantCleared)
			}
		})
	}
}
