...
func TestGameClientLinkUseManaPotionRestoresAndConsumes(t *testing.T) {
	...
	// Leave MP below max so the restore is observable. The fake store shared
	// its character pointer with the live player; the SQL store does not, so
	// the live character (already loaded from the store at EnterWorld) is
	// mutated directly, and nothing reloads the row before the assertion.
	live.Character.ReduceCurrentMP(20)
