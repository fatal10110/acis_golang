...
// beastShotSeed returns a newLinkedGameClientWithSkillsSeed seed closure
...
func beastShotSeed(t *testing.T, shotTemplate, shotObjectID int32, shotCount int) func(*fakeCharStore, *fakeItemStore) {
	t.Helper()
...
func TestGameClientLinkUseBeastSoulshotChargesSummonAndConsumes(t *testing.T) {
	...
	c, chars, _, state := newLinkedGameClientWithSkillsSeed(t, nil, beastShotSeed(t, 6645, shotObjectID, 10), 1)
